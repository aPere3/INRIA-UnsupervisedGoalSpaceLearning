...
"""
This scipt execute the experimental campaign.
...
class Experiment(experiment.BaseExperiment):
    ...
    def _run(self):
        ...
        if self._params['embedding'] is not in ['none', 'state']:
        ...
        elif self._params['embedding'] is 'none':
            pass
        else:
            samples_images = np.array([renderer.draw(state) for state in samples_states])
            samples_latents = embedder.transform(samples_images)

        # We perform the manifold quality measures
        self._log_lgr.info("Transforming training set to latents")
        if self._params['embedding'] is 'state':
        ...
        self._log_lgr.info("Sampling belief points")
        ...
        #self._log_lgr.info("Results: %s"% str(self._res_dict))
...
class Campaign(experiment.BaseExperiment):
    ...
    def _run(self):
        ...
        embedding_list = ["ae", "vae", "rfvae", "pixel", "isomap", "pca", "state", "none"] #6 hours
        environment_list = ["armball", "armtwoballs", "armarrow", "armtoolball"]
        noise_list = [0.0, 0.1, 0.25, 0.5, 0.7, 0.9]
        ...
        if self._params['test']:
            ...
            group_id = 1
            # We generate tuples of parameters
            params_itr = itertools.product(embedding_list, environment_list, noise_list)
            # We loop through the parameters
            for exp_id, (emb, env, nois) in enumerate(params_itr):
                params = copy.deepcopy(original_params)
                params.update({'embedding':emb, 'environment':env, 'noise':noise})
    ...
    def _init_expe(self, name, embedding, embedding_size, environment, sampling, sampling_size, noise, distraction,
                   deformation, outliers, random_motor_babbling, **kwargs):
        """
        ...
        if environment=="armball":
            params['env_config'] = dict(m_mins=[-1.] * 3, m_maxs=[1.] * 3, s_mins=[-1.] * 3, s_maxs=[1.] * 3,
                                    arm_lengths=[0.5, 0.3, 0.2], arm_angle_shift=0.5, arm_rest_state=[0.] * 3,
