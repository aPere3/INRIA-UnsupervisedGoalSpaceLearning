    ...
    def _run(self):
        ...
        if self._params['embedding'] is not 'none':
            ...
            if self._params['belief'] == 'hull':
                ...
                expected_latents = kde.resample(size=NB_POINTS_MANIFOLD_MEASURES).T
...
class Campaign(experiment.BaseExperiment):
    ...
    def _run(self):
        ...
        embedding_list = ["ae", "vae", "rfvae", "pixel", "isomap", "pca", "state", "none"]
        environment_list = ["armball", "armarrow"]
        noise_list = [0.0, 0.25, 0.5, 0.7]
        ...
        if self._params['test']:
            ...
            # Group 1: Different embeddings on different environments, on different noise
            self._log_lgr.info("Launching Group 1")
            group_id = 1
            # We generate tuples of parameters
            params_itr = itertools.product(embedding_list, environment_list, noise_list)
            # We loop through the parameters
            for exp_id, (emb, env, noi) in enumerate(params_itr):
                params = copy.deepcopy(original_params)
                params.update({'embedding':emb, 'environment':env, 'noise':noi})
